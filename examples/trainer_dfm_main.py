...
class DeepFactorizationMachineModel(torch.nn.Module):
    ...
    def __init__(self, ln_emb, emb_size, mlp_dims, dropout, device, embedding_bag_collection: EmbeddingBagCollection):
        super().__init__()
        self.device = device
        ...
        self.over_arch = OverArch(
            in_features=self.embed_output_dim,
            layer_sizes=mlp_dims,
            device=device,
        )
    ...
    def forward(self, D, X):
        ...
        """
        embedded_sparse = self.sparse_arch(X)
        # embed_x = torch.stack(embedded_sparse).transpose(0, 1)
        D = self.linear(D) + self.fm(embedded_sparse) + self.over_arch(embedded_sparse.flatten(start_dim=1))
        D = self.activation(D)
        return D
...
class DeepFMTrain(nn.Module):
    def __init__(
        self,
        deepfm_module,
    ) -> None:
        super().__init__()
        self.model = deepfm_module
...
def main(args) -> None:
    ...
    model = DeepFMTrain(DeepFactorizationMachineModel(ln_emb, 48, [64, 64, 64, 1], 0.5, device, EmbeddingBagCollection(tables=configs, device=torch.device("meta"))))
    ...
    datapipe = DataLoader(
        InMemoryBinaryCriteoIterDataPipe(
            stage="train",
            dense_paths=[os.path.join(template, "train_sparse_feats.npy")],
...
if __name__ == "__main__":
    args = parse_args(argparse.ArgumentParser(description="Arguments for DeepFM"))
    main(args)