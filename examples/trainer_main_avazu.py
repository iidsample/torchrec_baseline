...
# modify cd /home/ubuntu/ptorch/lib/python3.8/site-packages/torchrec/datasets

from torchrec.datasets.criteo import InMemoryBinaryCriteoIterDataPipe

# from avazu_data_loader import InMemoryBinaryCriteoIterDataPipe
...
class SparseArch(nn.Module):
    ...
    def forward(
        ...
        # print("After embedding_bag_collection")
...
def parse_args(parser):
    ...
    parser.add_argument(
        "--batch-size", type=int, default=16384, help="batch size to use for training"
    )
    parser.add_argument(
        "--world-size-trainers", type=int, default=8, help="For logging only"
    )
...
def main(args) -> None:
    ...
    model = DLRMTrain(
        DLRM(
            ...
            dense_arch_layer_sizes=[1024, 1024, 512, 256, 64, 48],
    ...
    datapipe = DataLoader(
        InMemoryBinaryCriteoIterDataPipe(
            stage="train",
            dense_paths=[os.path.join(template, "avazu_sample_dense.npy")],
            sparse_paths=[os.path.join(template, "avazu_sample_sparse.npy")],
            labels_paths=[os.path.join(template, "avazu_sample_target.npy")],
    ...
    while True and stop_iter < args.iter:
        try:
            ...
            logger.info(
                "zero_grad Time {}ms".format(
                    zero_grad_start.elapsed_time(zero_grad_stop)
                )
            )
            logger.info(
                "copy_batch_to_gpu Time {}ms".format(
                    copy_batch_to_gpu_start.elapsed_time(copy_batch_to_gpu_stop)
                )
            )
            logger.info(
                "wait_for_batch Time {}ms".format(
                    wait_for_batch_start.elapsed_time(wait_for_batch_stop)
                )
            )
            logger.info("dense Time {}ms".format(dense_start.elapsed_time(dense_stop)))
            logger.info(
                "sparse Time {}ms".format(sparse_start.elapsed_time(sparse_stop))
            )
            ...
            logger.info(
                "sparse_data_dist Time {}ms".format(
                    sparse_data_dist_start.elapsed_time(sparse_data_dist_stop)
                )
            )
            logger.info(
                "backward Time {}ms".format(backward_start.elapsed_time(backward_stop))
            )
            logger.info(
                "optimizer Time {}ms".format(
                    optimizer_start.elapsed_time(optimizer_stop)
                )
            )
    ...
    print(
        f"For a total of {stop_iter - 1} iterations, torchrec takes on average {statistics.mean(iter_list[1:])}ms with batch size of {batch_size}"
    )
    logger.info(
        f"For a total of {stop_iter - 1} iterations, torchrec takes on average {statistics.mean(iter_list[1:])}ms with batch size of {batch_size}"
    )
    logger.info(
        f"For a total of {stop_iter} iterations, torchrec takes {end_time - start_time}ms with batch size of {batch_size}"
    )
    if args.s3:
        s3_resource = uploadFile("recommendation-data-bagpipe")
        s3_resource.push_file(
            filename, f"{args.world_size_trainers}_trainers_torchrec/{filename}"
        )
