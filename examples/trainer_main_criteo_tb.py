...
def main(args) -> None:
    ...
    model = DLRMTrain(
        DLRM(
            ...
            dense_arch_layer_sizes=[1024, 1024, 512, 256, 64, 12],
    ...
    datapipe = DataLoader(
        InMemoryBinaryCriteoIterDataPipe(
            stage="train",
            dense_paths=[
                os.path.join(template, "criteo_terabyte_processed_dense_subset.npy")
            ],
            sparse_paths=[
                os.path.join(template, "criteo_terabyte_processed_sparse_subset.npy")
            ],
            labels_paths=[
                os.path.join(template, "criteo_terabyte_processed_target_subset.npy")
            ],
    ...
    if args.s3:
        ...
        s3_resource.push_file(
            filename,
            f"{args.world_size_trainers}_trainers_torchrec_terabyte/{filename}",
        )
